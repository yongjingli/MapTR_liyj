...
def debug_dataset(config_path):
    ...
    for i, data in enumerate(data_loader):
        ...
        gt_bboxes_list = [gt_bboxes.bbox for gt_bboxes in gt_bboxes_3d]
        gt_pts_list = [gt_bboxes.fixed_num_sampled_points for gt_bboxes in gt_bboxes_3d]
        gt_shifts_pts_list = [gt_bboxes.shift_fixed_num_sampled_points_v2 for gt_bboxes in gt_bboxes_3d]

        # 选择其中一个sample进行分析
        gt_bboxes = gt_bboxes_list[0]    # [N, 4]
        gt_pts = gt_pts_list[0]          # [N, 20，2]
        gt_shifts_pts = gt_shifts_pts_list[0]   # [N, 19, 20，2]

        # 绘制一个平面进行show
        gt_bboxes = gt_bboxes.detach().cpu().numpy()
        gt_pts = gt_pts.detach().cpu().numpy()
        gt_shifts_pts = gt_shifts_pts.detach().cpu().numpy()

        x_min = np.min(gt_pts[:, :, 0])
        x_max = np.max(gt_pts[:, :, 0])

        y_min = np.min(gt_pts[:, :, 1])
        y_max = np.max(gt_pts[:, :, 1])

        # 将坐标放大
        scale = 100

        x_bench = 1 - x_min  # 将x_min和y_min设置为1,方便在图像进行绘制
        y_bench = 1 - y_min  # 将x_min和y_min设置为1,方便在图像进行绘制
        ...
        img_show = np.ones((img_h, img_w, 3), np.uint8) * 255
        img_show2 = np.ones((img_h, img_w, 3), np.uint8) * 255
        num_l, num_p, _ = gt_pts.shape
        show_line_id = 0
        for i in range(num_l):
            ...
            for j in range(num_p):
                ...
                x = int(x * scale)
                y = int(y * scale)
                cv2.circle(img_show, (x, y), 10, color, -1)

        # show shift point line
        # 这里shift的含义: 对曲线共有两种组合,原始和flip的线段,0,1设置为这两种组合，其他设置为[-1000, -1000]
        # 对于封闭区间，选择的采样点数为20，那么组合的可能性有19种
        select_gt_shifts_pts = gt_shifts_pts[show_line_id]
        num_l, num_p, _ = select_gt_shifts_pts.shape
        for i in range(num_l):
            gt_pt = select_gt_shifts_pts[i]
            color = color_list[i]
            for j in range(num_p):
                _gt_pt = gt_pt[j]
                x, y = _gt_pt[0], _gt_pt[1]
                ...
                x = int(x * scale)
                y = int(y * scale)
                cv2.circle(img_show2, (x, y), 10, color, -1)
